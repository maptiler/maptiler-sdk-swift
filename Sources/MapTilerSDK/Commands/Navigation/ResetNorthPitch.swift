...
//
//  ResetNorthPitch.swift
//  MapTilerSDK
//

package struct ResetNorthPitch: MTCommand {
    ...
    package func toJS() -> JSString {
        ...
        if optionsString.isEmpty || optionsString == "{}" {
            return "\(MTBridge.mapObject).resetNorthPitch();"
        }

        optionsString = optionsString.replaceEasing()

        return "\(MTBridge.mapObject).resetNorthPitch(\(optionsString));"
    }
}

package struct ResetNorthPitchOptions: Encodable {
    var animationOptions: MTAnimationOptions?
